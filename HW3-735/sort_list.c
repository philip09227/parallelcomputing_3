...
#define MAX_LIST_SIZE   268435457
...
//
void* sort_list(void * id) {

    int i, level, my_id= (int) id; 
    int np, my_list_size; 
    ...
    pthread_barrier_wait(&barrier); // need to wait until all thread sort it's local list 
    ...
    for (level = 0; level < n; level++) {
	    ...
	    i_write = my_write_idx + my_search_count + (ptr[my_id]-my_own_idx); 
            // one thread can write at one time
...
// VS: ... parallel merge sort
    ...
    pthread_barrier_init(&barrier,&b_attr,num_threads);
    for (i = 0; i < num_threads; i++) {
        pthread_create(&threads[i],&attr,sort_list,(void *)i);

//        printf("each thread do works creating thread # %d \n", i);
    }
